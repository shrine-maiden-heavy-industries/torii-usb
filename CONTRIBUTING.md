...
Hey! Thanks for your interest in contributing to Torii USB!

A few important things to note up front:

* Contributions to the Torii USB source code are licensed under the [BSD-3-Clause], the full text of which can be found in the [`LICENSE`] file in the root of the [git repository].
* Contributions to the Torii USB documentation are licensed under the [CC-BY-SA 4.0], the full text of which can be found in the [`LICENSE.docs`] file in the root of the [git repository].
* When contributing to Torii USB and interacting with people in/around the project be sure to keep in mind the [Code Of Conduct].
...
The goal for Torii USB is to be able to be as friendly and powerful as possible, allowing for anyone of any skill level to get into hardware design, as such it is extremely important that we get feedback from users so we know what snags, sharp corners, unclear information, and unexpected behavior they encounter.

It is asked that you please report any and all problems you encounter when using Torii USB, this can be from small things such as typos or weird wording, and unclear/confusing messages and diagnostics, to larger things such as general useability and tool integration.

Reporting an issue with Torii USB is done through the [issue tracker], the issue templates should guide you through most of the needed information. A quick summary of what would be needed is as follows:

* The version of Torii USB being used, this can be done by running `python -c "import torii_usb;print(torii_usb.__version__)"`
...
We are trying to make Torii USB as widely useable and capable as possible, as such we love hearing about things that would help us achieve that goal.
...
The guidelines and instructions below are a best-effort attempt at describing the least-friction way of getting started with doing development and testing for Torii USB. If you notice any problems then please do [report an issue], and if you need any help then feel free to use the [discussion forum] or join the [IRC channel] and ask there.

### Required Tools

When working with the Torii USB code base, we require the following tools:
...
Other than that, your code editor and everything else is up to personal taste, so go ham. If you are having a hard time finding things like [Yosys], [sby], [Yices2], and [Bitwuzla], those, along with a whole lot of other tools are in the [OSS CAD Suite] builds. Instructions in that repository should help you get those set up so you can use them with Torii USB.

### Setting Up

1. [Fork] the repository to your GitHub account and clone it locally if you've not done so already previously.
1. Within the local repository, add this repo as a remote: `git remote add upstream https://github.com/shrine-maiden-heavy-industries/torii-usb`
...
The overall style guidelines for Torii USB are not super fixed or well defined at the moment, but the following things are required:
...
To run the Torii USB test suite, you can simply invoke `nox -s test`, this will do everything needed to set up Torii USB and then invoke it's test suite on the current state of the code base as it is locally.
...
* If the commit touches part of the core Torii USB package, try to use the full dotted path to the module. (e.g. `torii_usb.usb`, `torii_usb.interface.pipe`, etc. )
...
```text
torii_usb.full_devices: Added USB Mass Storage device

Added the USB Mass Storage device to the full_devices export.
```
...
Next, the `$function` section can be thought of something like a filesystem path segment that describes where this branch is focused, for instance, `usb3/device` if it's related to USB3 devices. If the changes are more wide-reaching than a specific sub-section, then this part can be omitted.
...
* `fix/usb3/request/timeout-fix`
* `docs/grammar-fixes`

### Submitting a Pull Request

> [!IMPORTANT]
> Torii USB uses a [git rebase] based workflow to maintain a linear history and avoid merge commits.

Once your changes are done, tested, and you're happy with them, then now to open a pull request against the Torii USB repository.
...
Once your changes are all up to date, make sure that all the tests pass (see the [Testing] section for more info), and then you can then open a [new pull request] against the Torii USB repository.
...
And with that, you're done! You've contributed to Torii USB!

## Documentation

The documentation for Torii USB uses [sphinx] with a few plugins, most notably [MyST], which allows us to use Markdown with some extensions to do the documentation rather than reStructuredText.

When contributing to the Torii USB documentation there are 2 key places it can be found, the "free text" documentation is located in the [`docs`] directory in the root of the repository and contains all the main markup for the overall layout and contents of the documentation site, where as the API and most library documentation is done via Python doc comments in the relevant source code itself.
...
[issue tracker]: https://github.com/shrine-maiden-heavy-industries/torii-usb/issues
[report an issue]: https://github.com/shrine-maiden-heavy-industries/torii-usb/issues
...
[fork]: https://github.com/shrine-maiden-heavy-industries/torii-usb/fork
...
[`docs`]: https://github.com/shrine-maiden-heavy-industries/torii-usb/blob/main/docs
[testing]: #testing
[new pull request]: https://github.com/shrine-maiden-heavy-industries/torii-usb/compare
